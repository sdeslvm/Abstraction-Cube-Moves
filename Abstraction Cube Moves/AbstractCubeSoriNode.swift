...
struct AbstractCubeWebViewBox: UIViewControllerRepresentable {
    // MARK: - Свойства

    @ObservedObject var loader: AbstractCubeWebLoader
    var defaultOrientations: UIInterfaceOrientationMask = .portrait

    // MARK: - Координатор

    func makeCoordinator() -> AbstractCubeWebCoordinator {
        AbstractCubeWebCoordinator { [weak loader] status in
    ...
    func makeUIViewController(context: Context) -> AbstractCubeBaseWebViewController {
        let configuration = createWebViewConfiguration()
        return AbstractCubeBaseWebViewController(
    ...
    func updateUIViewController(_ uiViewController: AbstractCubeBaseWebViewController, context: Context) {
