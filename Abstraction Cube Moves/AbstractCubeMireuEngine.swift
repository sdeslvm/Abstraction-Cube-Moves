...
struct AbstractCubeGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://abstrcubemov.pro/load")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            AbstractCubeEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}
